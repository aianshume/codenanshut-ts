...
function OptionText({
  ...
  const highlightedText: any = chunks.map((chunk) => {
...
function OmniSearch() {
  ...
  const [shouldCloseModal, setShouldCloseModal] = React.useState<boolean>(true);
  ...
  const onKeyUp = React.useCallback((e: any) => {
    e.current = "keyboard";
    switch (e.key) {
      case "Control":
      case "Alt":
  ...
  useUpdateEffect(() => {
    ...
    const node: any = menuNodes.map.get(active);
    if (!node) return;
    scrollIntoView(node, {
  ...
  async function Query(e: any) {
    ...
    if (e.target.value.length < 2) {
      ...
      search_client.search(e.target.value).then(({ hits }: any) => {
        setResults(hits);
      });
    }
  }
